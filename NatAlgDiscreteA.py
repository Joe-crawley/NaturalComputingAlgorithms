...
def WOA(n,num_cyc,N,b):
    Whales = [[random.randint(1, colours) for _ in range(n)] for _ in range(N)]         #Create random whales with random colouring
            ...
            if p < 0.5:
                for i in range(n):
                    ...
                    if abs(A[i]) < 1:       #if condition target whale is most fit whale
                        if abs(A[i]) > randomSearchValue:   #If condition greater then random generated value inherit colouring
